...
class TestSqlServerConnectionManager(TestCase):
    def setUp(self):
        self.conn_str = "DRIVER={ODBC Driver};SERVER=localhost;DATABASE=testdb"
        self.timeout = 5
        self.manager = SqlServerConnectionManager(self.conn_str, self.timeout)

    @patch("sqlpinger.engines.sqlserver.connection.pyodbc.connect")
